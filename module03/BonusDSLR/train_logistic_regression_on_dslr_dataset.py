...
svmgr = SVC()
Y_LINE = Y_GRIF_TR.astype('int').reshape(-1, 1)
svmgr.fit(X_TRAIN, Y_LINE)
YPRED_GRIF = svmgr.predict(X_TEST)

svmgr = SVC()
Y_LINE = Y_HUF_TR.astype('int').reshape(-1, 1)
svmgr.fit(X_TRAIN, Y_LINE)
YPRED_HUF = svmgr.predict(X_TEST)

svmgr = SVC()
Y_LINE = Y_RAVEN_TR.astype('int').reshape(-1, 1)
svmgr.fit(X_TRAIN, Y_LINE)
YPRED_RAVEN = svmgr.predict(X_TEST)

svmgr = SVC()
Y_LINE = Y_SLYT_TR.astype('int').reshape(-1, 1)
svmgr.fit(X_TRAIN, Y_LINE)
YPRED_SLYT = svmgr.predict(X_TEST)

ALL_PREDS = np.c_[tpl[1], YPRED_GRIF.reshape(-1, 1), YPRED_HUF.reshape(-1, 1), YPRED_RAVEN.reshape(-1, 1), YPRED_SLYT.reshape(-1, 1), tpl[7]]
DFSVM = pd.DataFrame(ALL_PREDS, columns=['Index', 'Gryffindor Probability', 'Hufflepuff Probability', 'Ravenclaw Probability', 'Slytherin Probability', 'Real House'])
print(DFSVM)
DFSVM.to_csv(path_or_buf='predict_scikit_svm.csv', sep=',',index=False)

from sklearn.ensemble import RandomForestClassifier
rfg = RandomForestClassifier(max_depth=2, random_state=0)
Y_LINE = Y_GRIF_TR.astype('int').reshape(-1, 1)
rfg.fit(X_TRAIN, Y_LINE)
YPRED_GRIF = rfg.predict(X_TEST)

rfg = RandomForestClassifier(max_depth=2, random_state=0)
Y_LINE = Y_HUF_TR.astype('int').reshape(-1, 1)
rfg.fit(X_TRAIN, Y_LINE)
YPRED_HUF = rfg.predict(X_TEST)

rfg = RandomForestClassifier(max_depth=2, random_state=0)
Y_LINE = Y_RAVEN_TR.astype('int').reshape(-1, 1)
rfg.fit(X_TRAIN, Y_LINE)
YPRED_RAVEN = rfg.predict(X_TEST)

rfg = RandomForestClassifier(max_depth=2, random_state=0)
Y_LINE = Y_SLYT_TR.astype('int').reshape(-1, 1)
rfg.fit(X_TRAIN, Y_LINE)
YPRED_SLYT = rfg.predict(X_TEST)

ALL_PREDS = np.c_[tpl[1], YPRED_GRIF.reshape(-1, 1), YPRED_HUF.reshape(-1, 1), YPRED_RAVEN.reshape(-1, 1), YPRED_SLYT.reshape(-1, 1), tpl[7]]
DFRF = pd.DataFrame(ALL_PREDS, columns=['Index', 'Gryffindor Probability', 'Hufflepuff Probability', 'Ravenclaw Probability', 'Slytherin Probability', 'Real House'])
print(DFRF)
DFRF.to_csv(path_or_buf='predict_scikit_random_forest.csv', sep=',',index=False)